...
class MongoDBClient:
    ...
    def find(
        ...
        res = self.collection.find(
            filter, projection=projection, skip=skip, limit=limit
        )
        if sort:
            res.sort(sort)
        data = [objectid_to_string(data) for data in res]
        return data

    def insert(
        self, data: Iterable, many: bool = False
    ...
    def update(self, filter: dict, update: dict, many: bool = False) -> UpdateResult:
        """
    ...
    def paginate_find(
        ...
        if per_page is not None:
            ...
            data = self.find(
                filter, projection=projection, skip=skip, limit=per_page, sort=sort
            )
        else:
            data = self.find(filter, projection=projection, sort=sort)
